...
* Потенциально завязаны на одном сервере, все имеют доступ ко всему без аутентификации
...
* **Аутентификация** с JWT токенами и т.п. было бы круто добавить, но у меня не было пока курса по backend разработке, я тут не силен
...
* **Масштабируемость и отказоустойчивость:** в текущей реализации артефакты хранятся локально, что создаёт единую точку отказа. В будущем планируется добавить поддержку объектного хранилища (S3 / MinIO) для надёжности и доступности моделей из любой точки.
* **Безопасность и аутентификация:** на данном этапе API открыт — это сознательное упрощение для MVP. Дальнейшее развитие предполагает внедрение JWT-аутентификации, разделение прав (admin / member / viewer) и аудит действий.
...
* **Хранилище артефактов (Artifact Store):** файловая система, где хранятся веса модели, конфигурации, файлы с метриками и т.д. Выбор файловой системы (вместо S3) обусловлен простотой реализации для MVP; в будущем планируется интеграция с облачными объектными хранилищами.
...
Текущая версия Model Registry является рабочим MVP, но имеет ряд ограничений, связанных с моим текущим опытом в backend-разработке:

* **Отсутствие аутентификации и авторизации** — API доступен без проверки прав. В будущем планируется добавить JWT-токены и ролевую модель (админ, участник команды, читатель).
* **Хранение артефактов на локальном диске** — упрощает развёртывание, но создаёт риск потери данных и не масштабируется. Следующий шаг — подключение S3-совместимого хранилища (MinIO, AWS S3) с использованием presigned URL для прямой загрузки.
* **Отсутствие полноценного audit log** — сейчас нет записи истории действий. Это важно для промышленной эксплуатации.

Эти улучшения я планирую изучить и реализовать после завершения курса в рамках своих проектов, чтобы углубить навыки backend-разработки и построения надёжных ML-инфраструктур.

[Источник дополнительных знаний](https://cloud.google.com/blog/products/ai-machine-learning/productionize-your-ml-models-and-pipelines-in-cloud)
